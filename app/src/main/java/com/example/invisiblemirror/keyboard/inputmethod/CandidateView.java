...
import com.example.invisiblemirror.keyboard.inputmethod.TextEntryState;
...
public class CandidateView extends View {
    ...
    private com.example.invisiblemirror.keyboard.inputmethod.SoftKeyboard mService;
        ...
        mGestureDetector = new GestureDetector(new GestureDetector.SimpleOnGestureListener() {
            ...
            public void onLongPress(MotionEvent me) {
                if (mSuggestions.size() > 0) {
                    if (me.getX() + /*mScrollX*/getScrollX() < mWordWidth[0] && /*mScrollX*/getScrollX() < 10) {
                        longPressFirstWord();
                    }
                }
            }
            ...
            public boolean onScroll(MotionEvent e1, MotionEvent e2,
                ...
                if (/*mScrollX*/getScrollX() < 0) {
                    //mScrollX = 0;
                	scrollTo(0, getScrollY());
                }
                if (distanceX > 0 && /*mScrollX*/getScrollX() + width > mTotalWidth) {                    
                    //mScrollX -= (int) distanceX;
                	scrollBy(-((int) distanceX), 0);
                }
                mTargetScrollX = /*mScrollX*/getScrollX();
                hidePreview();
                invalidate();
                return true;
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final int scrollX = /*mScrollX*/getScrollX();
        ...
        mTotalWidth = x;
        if (mTargetScrollX != /*mScrollX*/getScrollX()) {
            scrollToTarget();
        }
    }
    ...
    private void scrollToTarget() {
        if (mTargetScrollX > /*mScrollX*/getScrollX()) {
            ...
            if (/*mScrollX*/getScrollX() >= mTargetScrollX) {
    ...
    public void scrollPrev() {
        ...
        while (i < count) {
            if (mWordX[i] < /*mScrollX*/getScrollX() 
                    && mWordX[i] + mWordWidth[i] >= /*mScrollX*/getScrollX() - 1) {
                firstItem = i;
                break;
            }
            i++;
    ...
    public void scrollNext() {
        int i = 0;
        int targetX = /*mScrollX*/getScrollX();
        final int count = mSuggestions.size();
        int rightEdge = /*mScrollX*/getScrollX() + getWidth();
        while (i < count) {
    ...
    private void updateScrollPosition(int targetX) {
        if (targetX != /*mScrollX*/getScrollX()) {
            // TODO: Animate
    ...
    private void showPreview(int wordIndex, String altText) {
        ...
        if (oldWordIndex != mCurrentWordIndex || altText != null) {
            if (wordIndex == OUT_OF_BOUNDS) {
                ...
                mPopupPreviewX = mWordX[wordIndex] - mPreviewText.getPaddingLeft() - /*mScrollX*/getScrollX();
